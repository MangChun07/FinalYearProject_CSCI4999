...
class SplitBN(nn.Module):
    ...
    def forward(self, datas, syn_index):
        ...
        if syn_index != 0 and syn_index != batch_size:
            split = torch.split(datas, [int(syn_index), int(batch_size)-int(syn_index)], dim=0)
...
class ConvBlock(nn.Module):
    ...
    def forward(self, datas, labels:int):
        out = self.conv(datas)
...
class UpsampleBlock(nn.Module):
    def __init__(self, inplanes, outplanes, scale_factor=float(2)):
    ...
    def forward(self, datas):
        out = nn.functional.interpolate(datas, scale_factor=float(self.scale_factor), mode='bilinear')
        out = self.conv(out)
        # out = self.leaky_relu(out)
        return out
...
class HandNetInitial(nn.Module):
    def __init__(self):
        ...
        self.upsample32 = UpsampleBlock(512, 256, scale_factor=float(2))
        self.upsample16 = UpsampleBlock(256, 128, scale_factor=float(2))
        self.upsample8 = UpsampleBlock(128, 64, scale_factor=float(2))
        self.upsample4 = UpsampleBlock(64, 32, scale_factor=float(2))
        self.upsample2 = UpsampleBlock(32, 16, scale_factor=float(2))

        # self.global_upsample2 = UpsampleBlock(128, 64, scale_factor=float(2))
        # self.global_upsample4 = UpsampleBlock(128, 32, scale_factor=float(4))
        # self.global_upsample8 = UpsampleBlock(128, 16, scale_factor=float(8))
        # self.global_upsample16 = UpsampleBlock(128, 16, scale_factor=float(8))
    ...
    def forward(self, x, y, masks, label):
        ...
        result16_max = nn.functional.interpolate(result16_max, scale_factor=float(2), mode='bilinear')
        f_u16, _ = self.global_context2(f_u16)
        
        mask_temp = F.adaptive_avg_pool2d((mask[:,2,:,:]), (40,40)) + 1
        ...
        result8_max = nn.functional.interpolate(result8_max, scale_factor=float(2), mode='bilinear')
        f_u8, _ = self.global_context3(f_u8)

        mask_temp = F.adaptive_avg_pool2d((mask[:,2,:,:]), (80,80)) + 1
        mask_temp = torch.unsqueeze(mask_temp, 1)
        ...
        result4_max = nn.functional.interpolate(result4_max, scale_factor=float(2), mode='bilinear')

        mask_temp = F.adaptive_avg_pool2d((mask[:,2,:,:]), (160,160)) + 1
        mask_temp = torch.unsqueeze(mask_temp, 1)
        ...
        result2_max = nn.functional.interpolate(result2_max, scale_factor=float(2), mode='bilinear')
